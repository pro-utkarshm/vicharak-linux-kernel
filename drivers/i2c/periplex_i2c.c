...
int read_data_for_i2c(struct periplex_device *dev, char *message,
                      const int len)
{
    data_from_usr = len;
                   ...
                   union i2c_smbus_data *data)
{
    ...
    case I2C_SMBUS_BYTE:
        ...
        if (read_write == I2C_SMBUS_READ && data)
        {
            ...
            pr_info("read_data_i2c[0] %d\n", read_data_i2c[0]);

            if (read_data_i2c[0] == 255)
            {
                pr_info("No device present\n");
                ret = -ENXIO; // No such device or address
            }
            else
            {
                pr_info("Device present\n");
            }
            kfree(read_data_i2c);
        }
        break;
...
*/
static int periplex_i2c_probe(struct platform_device *pdev)
{
    int ret;
    int retval;
    int divider;
    ...
    struct periplex_device *i2c_d;

    i2c_d = kzalloc(sizeof(struct periplex_device), GFP_KERNEL);
    if (!i2c_d)
        return -ENOMEM;
    ...
    i2c_d->pdev = pdev;
    i2c_d->data = i2c_adapter;
    i2c_d->get_periplex_data = read_data_for_i2c;

    /* This is mandatory part to register device with periplex */
    platform_set_drvdata(pdev, i2c_d);
...
*/
static int periplex_i2c_remove(struct platform_device *pdev)
{
    struct periplex_device *i2c_d = platform_get_drvdata(pdev);
    struct i2c_adapter *i2c_adapter = i2c_d->data;
...
struct platform_driver periplex_i2c_driver = {
...
module_platform_driver(periplex_i2c_driver);

MODULE_ALIAS("platform:i2c");
